...
public class Carnivore3 extends CVagile {
	...
	public Carnivore3(int _id, String _name, ObjectType _objectType, Point3D _position, int age, Point3D size, int scrWidth, int scrHeight, Color color, AnimalInfo animalInfo)
	{		
	...
	public void update(ArrayList<CWorldObject> worldObjects, CEnvironment env) 
	 {
    	...
    	for (int i=0; i<worldObjects.size(); i++)
        {
    		...
    		if(worldObjects.get(i).objectType == ObjectType.agent)
        	{
    			if ((this.id != worldObjects.get(i).id))
    			{
	    			...
	    			{	
	    				//System.out.println("worldObjects.get(i).name: " + worldObjects.get(i).name);
	    				//System.out.println("info.food: " + info.food);
	    				Carnivore2 food = (Carnivore2) worldObjects.get(i);
	    			...
	    			if(worldObjects.get(i) instanceof Carnivore4)
	    			{	
	    				//System.out.println("Carnivore2 aroudn me!" + worldObjects.get(i).name);
	    				Carnivore4 pred = (Carnivore4) worldObjects.get(i);
		...
		if (animalState == AnimalState.FLEEING)
		{
			...
			{
				// the natural predator (upper foodchain) must have priority, so must replace the hunt for food
    			if(worldObjects.get(predatorIndex) instanceof Carnivore4)
    			{
    				setThrust((int) info.fleeingThrust);
    				energyTest(info.energyLoss);
    				Carnivore4 pred = (Carnivore4) worldObjects.get(predatorIndex);
		...
		{				
			if(preyIndex != -1)
			{
    			...
    			{		
					...
					Carnivore2 hbv = (Carnivore2) worldObjects.get(preyIndex);
    	...
    	if(((rX) < screenWidth) && (rY) < screenHeight)
    	{
        	...
        	{       			
            	...
            	if(!outOfWorld)
            	{
	            		...
	            		Carnivore3 p = new Carnivore3( TrophicNetwork.AgentIDInc++, info.commonName, objectType, rLoc, 1, size, screenWidth, screenHeight, pColor, info);
