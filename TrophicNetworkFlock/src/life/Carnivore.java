...
public class Carnivore extends CVagile {
	...
	public Carnivore(int _id, String _name, ObjectType _objectType, Point3D _position, int age, Point3D size, int scrWidth, int scrHeight, Color color, AnimalInfo animalInfo)
	{		
    	...
    	for (int i=0; i<worldObjects.size(); i++)
        {
    		...
    		if(worldObjects.get(i).objectType == ObjectType.agent)
        	{
    			if ((this.id != worldObjects.get(i).id))
    			{
	    			...
	    			{	
	    				//System.out.println("Carnivore aroudn me!" + worldObjects.get(i).name);
		...
		{				
			if(preyIndex != -1)
			{
    			...
    			{		
					...
					Herbivore hbv = (Herbivore) worldObjects.get(preyIndex);
	    	...
	    	if(((rX) < screenWidth) && (rY) < screenHeight)
	    	{
	        	...
	        	{     			
	            	...
	            	if(!outOfWorld)
	            	{
		            		...
		            		Carnivore p = new Carnivore( TrophicNetwork.AgentIDInc++, info.commonName, objectType, rLoc, 1, size, screenWidth, screenHeight, pColor, info);
