...
public class Carnivore4 extends CVagile {
	...
	public Carnivore4(int _id, String _name, ObjectType _objectType, Point3D _position, int age, Point3D size, int scrWidth, int scrHeight, Color color, AnimalInfo animalInfo)
	{		
	...
	public void update(ArrayList<CWorldObject> worldObjects, CEnvironment env) 
	 {
    	...
    	for (int i=0; i<worldObjects.size(); i++)
        {
    		...
    		if(worldObjects.get(i).objectType == ObjectType.agent)
        	{
    			if ((this.id != worldObjects.get(i).id))
    			{
	    			...
	    			{	
	    				Carnivore3 food = (Carnivore3) worldObjects.get(i);
	    			...
	    			if(worldObjects.get(i) instanceof Carnivore5)
	    			{	
	    				//System.out.println("Carnivore2 aroudn me!" + worldObjects.get(i).name);
	    				Carnivore5 pred = (Carnivore5) worldObjects.get(i);
		...
		if (animalState == AnimalState.FLEEING)
		{
			...
			{
				// the natural predator (upper foodchain) must have priority, so must replace the hunt for food
    			if(worldObjects.get(predatorIndex) instanceof Carnivore5)
    			{
    				setThrust((int) info.fleeingThrust);
    				energyTest(info.energyLoss);
    				Carnivore5 pred = (Carnivore5) worldObjects.get(predatorIndex);
		...
		{				
			if(preyIndex != -1)
			{
    			...
    			{		
					...
					Carnivore3 hbv = (Carnivore3) worldObjects.get(preyIndex);
	    	...
	    	if(((rX) < screenWidth) && (rY) < screenHeight)
	    	{
	        	...
	        	{       			
	            	...
	            	if(!outOfWorld)
	            	{
	            		...
	            		Carnivore4 p = new Carnivore4( TrophicNetwork.AgentIDInc++, info.commonName, objectType, rLoc, 1, size, screenWidth, screenHeight, pColor, info);
