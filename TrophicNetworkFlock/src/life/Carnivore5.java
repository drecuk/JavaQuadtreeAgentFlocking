...
public class Carnivore5 extends CVagile {
	...
	public Carnivore5(int _id, String _name, ObjectType _objectType, Point3D _position, int age, Point3D size, int scrWidth, int scrHeight, Color color, AnimalInfo animalInfo)
	{		
		...
		// ***** NETWORK MAPPING
				...
				String[] nStr = {"C5-"+this.id, Integer.toString((int)speed), strColor};
	...
	public void update(ArrayList<CWorldObject> worldObjects, CEnvironment env) 
	 {
    	...
    	for (int i=0; i<worldObjects.size(); i++)
        {
    		...
    		if(worldObjects.get(i).objectType == ObjectType.agent)
        	{
    			if ((this.id != worldObjects.get(i).id))
    			{
	    			...
	    			{	
	    				Carnivore4 food = (Carnivore4) worldObjects.get(i);
	    			...
	    			if(worldObjects.get(i) instanceof Carnivore6)
	    			{	
	    				//System.out.println("Carnivore2 aroudn me!" + worldObjects.get(i).name);
	    				Carnivore6 pred = (Carnivore6) worldObjects.get(i);
		...
		if (animalState == AnimalState.FLEEING)
		{
			...
			{
				// the natural predator (upper foodchain) must have priority, so must replace the hunt for food
    			if(worldObjects.get(predatorIndex) instanceof Carnivore6)
    			{
    				Carnivore6 pred = (Carnivore6) worldObjects.get(predatorIndex);
		...
		{				
			if(preyIndex != -1)
			{
    			...
    			{		
					...
					Carnivore4 hbv = (Carnivore4) worldObjects.get(preyIndex);
	    	...
	    	if(((rX) < screenWidth) && (rY) < screenHeight)
	    	{
	        	...
	        	{       			
	            	...
	            	if(!outOfWorld)
	            	{
		            		...
		            		Carnivore5 p = new Carnivore5( TrophicNetwork.AgentIDInc++, info.commonName, objectType, rLoc, 1, size, screenWidth, screenHeight, pColor, info);
