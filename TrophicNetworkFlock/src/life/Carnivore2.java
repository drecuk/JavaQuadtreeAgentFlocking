...
public class Carnivore2 extends CVagile {
	...
	public Carnivore2(int _id, String _name, ObjectType _objectType, Point3D _position, int age, Point3D size, int scrWidth, int scrHeight, Color color, AnimalInfo animalInfo)
	{		
		...
		String[] nStr = {"C2-"+this.id, Integer.toString((int)speed), strColor};
	...
	public void update(ArrayList<CWorldObject> worldObjects, CEnvironment env) 
	 {
		...
		if(!reproduced)
			if(age >= info.reproductionAge)
			{
				...
				if ((random.nextFloat() < info.reproductionProbability) && (fitness >= info.reproductionFitness ))				
					reproduce(info.seedCount);
				reproduced = true;
			}
		...
		*/
    	...
    	for (int i=0; i<worldObjects.size(); i++)
        {
    		...
    		if(worldObjects.get(i).objectType == ObjectType.agent)
        	{
    			if ((this.id != worldObjects.get(i).id))
    			{
	    			...
	    			{	
	    				Carnivore herbi = (Carnivore) worldObjects.get(i);
	    					    				
	    				if(getDistance(herbi.position, position) < info.eyeSight)
	    				{
	    			...
	    			if(worldObjects.get(i) instanceof Carnivore3)
	    			{	
	    				//System.out.println("Carnivore2 aroudn me!" + worldObjects.get(i).name);
	    				Carnivore3 pred = (Carnivore3) worldObjects.get(i);
		...
		if (animalState == AnimalState.FLEEING)
		{
			...
			{
				// the natural predator (upper foodchain) must have priority, so must replace the hunt for food
    			if(worldObjects.get(predatorIndex) instanceof Carnivore3)
    			{
    				setThrust((int) info.fleeingThrust);
    				energyTest(info.energyLoss);
    				Carnivore3 pred = (Carnivore3) worldObjects.get(predatorIndex);
		...
		{				
			if(preyIndex != -1)
			{
    			...
    			{		
					...
					Carnivore hbv = (Carnivore) worldObjects.get(preyIndex);
	    	...
	    	if(((rX) < screenWidth) && (rY) < screenHeight)
	    	{
	        	...
	        	{     			
	            	...
	            	if(!outOfWorld)
	            	{
		            		...
		            		Carnivore2 p = new Carnivore2( TrophicNetwork.AgentIDInc++, info.commonName, objectType, rLoc, 1, size, screenWidth, screenHeight, pColor, info);
    ...
    public void paint(Graphics g)
    {
        ...
        g.drawLine(position.x, position.y, (int)(position.x + Math.cos(angle*Math.PI/180)*5.0f), (int)(position.y + Math.sin(angle*Math.PI/180)*5.0f));
