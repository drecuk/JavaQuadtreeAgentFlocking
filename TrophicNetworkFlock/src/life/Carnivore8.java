...
public class Carnivore8 extends CVagile {
	...
	public Carnivore8(int _id, String _name, ObjectType _objectType, Point3D _position, int age, Point3D size, int scrWidth, int scrHeight, Color color, AnimalInfo animalInfo)
	{		
		...
		String[] nStr = {"C8-"+this.id, Integer.toString((int)speed), strColor};
	...
	public void update(ArrayList<CWorldObject> worldObjects, CEnvironment env) 
	 {
		...
		if (animalState == AnimalState.FLEEING)
		{
			...
			{
				// the natural predator (upper foodchain) must have priority, so must replace the hunt for food
    			if(worldObjects.get(predatorIndex) instanceof Carnivore2)
    			{
    				Carnivore2 pred = (Carnivore2) worldObjects.get(predatorIndex);
    				
    				setThrust((int) info.fleeingThrust);
    				energyTest(info.energyLoss);
    				energyToReproduce += pred.animalInfo.energy;
		...
		{				
			if(preyIndex != -1)
			{
    			...
    			{		
					...
					Carnivore7 hbv = (Carnivore7) worldObjects.get(preyIndex);
		...
		if(((rX) < screenWidth) && (rY) < screenHeight)
		{
	    	...
	    	{    			
	        	...
	        	if(!outOfWorld)
	        	{
	            		...
	            		Carnivore8 p = new Carnivore8( TrophicNetwork.AgentIDInc++, info.commonName, objectType, rLoc, 1, size, screenWidth, screenHeight, pColor, info);
