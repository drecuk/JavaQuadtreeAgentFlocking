...
public class Human extends CVagile {
	...
	ArrayList<Point3D> clanIndices = new ArrayList<Point3D>(); 	// for storing indices of friends for collective behv.
	
	public Human(int _id, String _name, ObjectType _objectType, Point3D _position, int age, Point3D size, int scrWidth, int scrHeight, Color color, AnimalInfo humanInfo)
	{
	...
	public void update(ArrayList<CWorldObject> worldObjects, CEnvironment env) {
    			...
    			if ((this.id != worldObjects.get(i).id))	// avoid calculating myself
    			{
	    			...
	    			if(worldObjects.get(i) instanceof Bird)
	    			{
	    				...
	    				if(Utility.distance(position.x, position.y, friend.position.x, friend.position.y) < info.eyeSight)
							...
							{	
								//System.out.println(" ******* Found my type " + worldObjects.get(i).name);
								clanIndices.add(friend.position);
		...
		*/
		...
		//angle = Math.round(Math.atan2(yMove,xMove)*180/Math.PI);
		
		xMove += Math.cos(angle*Math.PI/180) * 2 * thrust;
		yMove += Math.sin(angle*Math.PI/180) * 2 * thrust;
	...
	public void paint(Graphics g) {
        ...
        /*
        for(int i=0; i<clanIndices.size();i++)
        {
        	g.drawLine(position.x,  position.y, clanIndices.get(i).x, clanIndices.get(i).y);
        }
        */
        if (showInfo == true)
        {
