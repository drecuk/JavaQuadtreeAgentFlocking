...
public class Herbivore extends CVagile {
	...
	// prey
	int preyIndex;
	int preyNodeIndex;	// the node's array index in the quadtree 
	int predatorIndex;
	...
	public Herbivore(int _id, String _name, ObjectType _objectType, Point3D _position, int age, Point3D size, int scrWidth, int scrHeight, Color color, AnimalInfo animalInfo)
	{		
		super(_id, _name, _objectType, _position, (int) animalInfo.eyeSight);
		this.category = animalInfo.predatorCategory; // maybe upperCategory and lowerCategory? 
		...
		String[] nStr = {"H-"+this.id, Integer.toString((int)speed), strColor};
	...
	public void update(ArrayList<CWorldObject> worldObjects, CEnvironment env) 
	{
		...
		float tempEmitter = 0.0f;
		 // loop through range query objects
    	for (int i=0; i<worldObjects.size(); i++)
        {
    		...
    		//	System.out.println(worldObjects.get(i).objectType);
        	...
        	{	
    			...
    			if(new String(em.name).equals("TemperatureEmitter"))
    			{
    				// cast it to the type
    		...
    		if(worldObjects.get(i).objectType == ObjectType.agent)
        	{
    			if ((this.id != worldObjects.get(i).id))
    			{
    				...
    				//System.out.println("compare::" + worldObjects.get(i).name + "==" + info.food);
    				
	    			if(new String(worldObjects.get(i).name).equals(info.food))	// if the object is a food according to my type (info.food)
	    			{
	    				// once found, cast it to the coder's custom type based on the name
	    				Plant pl = (Plant) worldObjects.get(i);
	    				
	    				// if it is near enough, change states
	    				if(getDistance(pl.position, position) < info.eyeSight)
	    				{
	    					//System.out.println("found plant: " + getDistance(pl.position, position));
		    				preyIndex = i;
		    				animalState = AnimalState.HUNTING;
		    				
		    				// ***** NETWORK MAPPING
		    				String[] neStr = {"true"}; // true for directed
		    				NetworkInfo.addEdge(new NAgentEdge(this.id, pl.id, neStr));
	    			...
	    			if(worldObjects.get(i) instanceof Carnivore)
	    			{	
	    				//System.out.println("Carnivore2 aroudn me!" + worldObjects.get(i).name);
	    				Carnivore pred = (Carnivore) worldObjects.get(i);
		...
		if (animalState == AnimalState.FLEEING)
		{
			...
			{
				// the natural predator (upper foodchain) must have priority, so must replace the hunt for food
    			if(worldObjects.get(predatorIndex) instanceof Carnivore)
    			{
    				setThrust((int) info.fleeingThrust);
    				energyTest(info.energyLoss);
    				Carnivore pred = (Carnivore) worldObjects.get(predatorIndex);
		...
		{	
			if(preyIndex != -1)
			{
    			...
    			{	
    				// once found, cast it to the coder's custom type based on the name
    				Plant pl = (Plant) worldObjects.get(preyIndex);
    				
    				float fX =	pl.position.x - position.x;	// target x
    				float fY = 	pl.position.y - position.y;	// target y
    				
    				angle = Math.round(Math.atan2(fY,fX)*180/Math.PI);					// angle towards prey
    				
    				visibleVec = vRotate2D((int) angle+90, pl.position.x, pl.position.y); // get visibility vector
    				
    				// if it is near enough, change states.
    				if(getDistance(pl.position, position) < info.feedingDistance)
    				{
    					preyFound = false;
    					energyTest(pl.plantInfo.energy);
    					energyToReproduce += pl.plantInfo.energy;
    					...
    					{	    					
    						...
    						//worldObjects.remove(worldObjects.get(preyIndex));
    						
    						//System.out.println("agent removed:" + worldObjects.get(preyIndex).id);
    					} catch (Exception ex) { }
		...
		position.y = yMove;
		//viewable();
		//System.out.println("[" + animalState + "]e:" + energy + "::(" + preyIndex + ")::" + pos.x + "," + pos.y + " | angle:"+ rotAngle + " | thrust:" + thrust);
	 ...
	 public void reproduce(int offspring)
	 {
    	...
    	double rX = position.x + rndDist * Math.cos(rndAngle*Math.PI/180);
    	double rY = position.y + rndDist * Math.sin(rndAngle*Math.PI/180);
    	//System.out.println("angle:" + rndAngle + " | " + Math.ceil(rndDist * Math.sin(rndAngle)));
    	...
    	if(((rX) < screenWidth) && (rY) < screenHeight)
    	{
        	...
        	{       			
            	...
            	if(!outOfWorld)
            	{
	            		...
	            		Herbivore p = new Herbivore( TrophicNetwork.AgentIDInc++, info.commonName, objectType, rLoc, 1, size, screenWidth, screenHeight, pColor, info);
    ...
    public void paint(Graphics g)
    {
        ...
        g.drawLine(position.x, position.y, (int)(position.x + Math.cos(angle*Math.PI/180)*3.0f), (int)(position.y + Math.sin(angle*Math.PI/180)*3.0f));
	...
	public int getPreyIndex()
	{
		...
		for (int i=0; i<Plants.size(); i++)
        {
			...
			{
    			//float dX = ((Plant)Collections.get(i)).pLoc.x - pos.x;
    			//float dY = ((Plant)Collections.get(i)).pLoc.y - pos.y;
